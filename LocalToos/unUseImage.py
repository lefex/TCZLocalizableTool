...
# 将要解析的项目名称 
DESPATH = "/Users/wangsuyan/Desktop/project/Kmart"
...
def find_image_name(filePath):
    ...
    for index, line in enumerate(f):
        ...
        if '.xib' in filePath or 'storyboard' in filePath:
            regx = r'image="(.+?).(?:png|jpg|jpeg|gif)"'
        else:
...
def find_from_file(path):
    ...
    for aCompent in paths:
        ...
        if os.path.isdir(aPath):
            ...
            un_use_image(aPath)
        elif os.path.isfile(aPath) and (ext=='.m' or ext=='.xib' or ext=='.storyboard'):
